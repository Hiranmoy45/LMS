...
if(isset($_REQUEST['requpdate'])){
    ...
    if(($_REQUEST['course_id'] == "") || ($_REQUEST['course_name'] == "") || ($_REQUEST['course_desc'] == "") || ($_REQUEST['course_author'] == "") || ($_REQUEST['course_duration'] == "")){
       ...
       $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2" role="alert">Fill All Fields</div>';
        ...
        $cprice = "0";
        $coriginalprice = "0";
        $ckeyword= $_REQUEST['course_keyword'];
        $cimg = '../image/courseimg/'. $_FILES['course_img']['name'];


        $sql = "UPDATE freecourse SET course_id = '$cid', course_name = '$cname', course_desc = '$cdesc',course_author = '$cauthor', course_duration = '$cduration',course_price = '$cprice',course_original_price = '$coriginalprice', course_img = '$cimg', course_key ='$ckeyword' WHERE course_id = '$cid'";
         if($conn->query($sql) == TRUE) {
            //  below msg display on from submit success
            $msg = '<div class="alert alert-success col-sm-6 ml-5 mt-2" role="alert">Course Update Succesfully</div>';
         } else {
...
<div class="col-sm-6 mx-3 jumbotron">
    ...
    <?php
        ...
        $sql = "SELECT * FROM freecourse WHERE course_id = {$_REQUEST['id']}";
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="course_keyword" name="course_keyword"value="<?php if(isset($row['course_key'])) { echo $row['course_key']; } ?> ">
        </div>
        <div class="form-group">
